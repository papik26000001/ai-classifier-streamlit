...
if fl:
    ...
    st.info(f"☁️ Результат створено у пам'яті Streamlit Cloud. "
                "Обов’язково завантажте файл через кнопку вище — "
                "після перезапуску сторінки файл буде втрачено./`{fl}`")
else:
    # st.info(f"📁 Результат також збережено локально у `{out_path.name}`/`{fl}` ")
